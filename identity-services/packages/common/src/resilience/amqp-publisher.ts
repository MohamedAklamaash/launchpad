...
export class ResilientAmqpPublisher {
    ...
    publish(
        ...
        if (!this.channel) {
            ...
            if (this.buffer.length >= this.maxBufferSize) {
                ...
                this.log.warn({ routingKey: dropped?.routingKey }, "Buffer full – dropped oldest message");
            }
    ...
    private doPublish(msg: BufferedMessage): void {
        ...
        const ok = this.channel.publish(
            ...
            msg.routingKey,
            msg.content,
            { contentType: "application/json", persistent: true, ...msg.options }
        );
        if (!ok) {
    ...
    private scheduleReconnect(): void {
        ...
        if (this.maxRetries > 0 && this.retryCount >= this.maxRetries) {
            this.log.error({ retryCount: this.retryCount }, "Max reconnect attempts reached – giving up");
            return;
        }
