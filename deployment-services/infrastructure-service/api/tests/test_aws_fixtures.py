...
EKS_FILLED_FIELDS = {"vpc_id", "cluster_arn", "alb_dns", "ecr_repository_url"}


def test_synthesize_ecs_outputs_fills_all_eight_fields():
    outputs = aws_fixtures.synthesize_environment_outputs(_FakeInfra(), "us-east-1", ComputeType.ECS_FARGATE)
    assert set(outputs) == ALL_FIELDS
    assert all(outputs[k] for k in ALL_FIELDS)


def test_synthesize_eks_outputs_fills_only_eks_subset():
    outputs = aws_fixtures.synthesize_environment_outputs(_FakeInfra(), "us-east-1", ComputeType.EKS)
    assert set(outputs) == ALL_FIELDS
    assert all(outputs[k] for k in EKS_FILLED_FIELDS)
    assert all(outputs[k] is None for k in ALL_FIELDS - EKS_FILLED_FIELDS)
    assert ":cluster/infra-" in outputs["cluster_arn"]
    assert outputs["cluster_arn"].startswith("arn:aws:eks:")
