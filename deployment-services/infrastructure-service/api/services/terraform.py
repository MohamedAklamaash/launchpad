...
class TerraformService:
    ...
    def run_terraform(cls, action_args: list[str], credentials: dict, env_vars: dict) -> dict:
        try:
            ...
            subprocess.run(init_cmd, cwd=TF_WORKING_DIR, check=True, capture_output=True, text=True)
            ...
            result = subprocess.run(cmd, cwd=TF_WORKING_DIR, check=True, capture_output=True, text=True, env=process_env)
            
            logger.info("Terraform execution successful.")
            return {"success": True, "output": result.stdout}

        except subprocess.CalledProcessError as e:
            logger.error(f"Terraform execution failed. Exit code: {e.returncode}")
